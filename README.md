...
![tvOS](https://img.shields.io/badge/tvOS-18.0%2B-000000?style=flat&logo=apple&logoColor=white)
...
## Features
- **StoreKit 2 lifetime unlock** (`galleryglow.lifetime`) for a one-time **$9.99** purchase
...
## Purchase setup
The app expects a non-consumable StoreKit product:

- Product ID: `galleryglow.lifetime`
- Reference name: `Gallery Glow Lifetime`
- Price: `$9.99`
- Family Sharing: enabled

For local testing, use `Gallery Glow/GalleryGlow.storekit`. In App Store Connect, create the same non-consumable product ID and set the `$9.99` price tier before submitting the build.
...
- `galleryglow://painting/Hopper%2FNighthawks`
...
2. Add a `Painting(...)` entry in `Gallery Glow/PaintingData.swift`.
   - The Top Shelf extension compiles `PaintingData.swift` directly, so it picks up the change automatically.
3. Create a Top Shelf-friendly image file named by replacing `/` with `_` (or run `./generate_topshelf_images.sh`).
   - Example: `Hopper/Nighthawks` → `Hopper_Nighthawks.jpg`
